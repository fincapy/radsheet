...
export function writeVarintUnsigned(value, outArray) {
  let v = value >>> 0;
  while (v > 0x7F) { outArray.push((v & 0x7F) | 0x80); v >>>= 7; }
  outArray.push(v);
}

export function readVarintUnsigned(bytes, startIndex) {
  let shift = 0, result = 0, index = startIndex, current;
  do {
    current = bytes[index++];
...
export const ChunkCodec = {
  ...
  encodeDenseChunk(denseChunk, globalStringTable) {
    ...
    let stringCount = 0;
    for (let i = 0; i < CELLS_PER_CHUNK; i++) if (denseChunk.tagByLocalIndex[i] === 2) stringCount++;
  ...
  decodeDenseChunk(encodedBytes, globalStringTable) {
    let readIndex = 0;
    const typeByte = encodedBytes[readIndex++], versionByte = encodedBytes[readIndex++];
    if (typeByte !== 0x44 || versionByte !== 0x01) throw new Error('Invalid dense chunk header');

    let rleLength; [rleLength, readIndex] = readVarintUnsigned(encodedBytes, readIndex);
    ...
    let numericCount; [numericCount, readIndex] = readVarintUnsigned(encodedBytes, readIndex);
    ...
    let stringCount; [stringCount, readIndex] = readVarintUnsigned(encodedBytes, readIndex);
    for (let decoded = 0, i = 0; i < CELLS_PER_CHUNK && decoded < stringCount; i++) {
      if (dense.tagByLocalIndex[i] === 2) {
        let stringId; [stringId, readIndex] = readVarintUnsigned(encodedBytes, readIndex);
  ...
  encodeSparseChunk(sparseChunk, globalStringTable) {
    ...
    const sortedEntries = Array.from(sparseChunk.localIndexToValue.entries())
      .sort((a, b) => a[0] - b[0]);
    ...
    for (const [localIndex, jsValue] of sortedEntries) {
      ...
      isFirst = false; previousLocalIndex = localIndex;
      ...
      if (typeof jsValue === 'number') {
        ...
        merged.set(before, 0); merged.set(numBytes, before.length);
  ...
  decodeSparseChunk(encodedBytes, globalStringTable) {
    ...
    let entryCount; [entryCount, readIndex] = readVarintUnsigned(encodedBytes, readIndex);
    const sparse = createSparseChunk();

    let currentLocalIndex = 0;
    for (let n = 0; n < entryCount; n++) {
      let delta; [delta, readIndex] = readVarintUnsigned(encodedBytes, readIndex);
      currentLocalIndex += delta;

      const valueTag = encodedBytes[readIndex++];

      if (valueTag === 1) { // number
      ...
      } else if (valueTag === 3) { // boolean
        const value = !!encodedBytes[readIndex++];
        sparse.localIndexToValue.set(currentLocalIndex, value);
      } else if (valueTag === 2) { // string
        let stringId; [stringId, readIndex] = readVarintUnsigned(encodedBytes, readIndex);
...
export class ChunkStore {
  constructor({ databaseName = 'SheetDB', chunkStoreName = 'chunks', metaStoreName = 'meta' } = {}, idbFactory) {
  ...
  async _openIfNeeded() {
    ...
    this.database = await new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = () => {
        ...
        if (!db.objectStoreNames.contains(this.chunkStoreName)) db.createObjectStore(this.chunkStoreName);
...
export class Sheet {
  ...
  getValue(globalRowIndex, globalColIndex) {
    const chunk = this._getHotChunkSynchronously(globalRowIndex, globalColIndex, /*createIfMissing=*/ false);
  ...
  setValue(globalRowIndex, globalColIndex, value) {
    ...
    if (value === '' || value == null) { this.deleteValue(globalRowIndex, globalColIndex); return; }

    let chunk = this._getHotChunkSynchronously(globalRowIndex, globalColIndex, /*createIfMissing=*/ true);
    const localIndex = computeLocalIndexWithinChunk(globalRowIndex, globalColIndex);
    chunk.isDirty = true;
    ...
    if (chunk.kind === 'sparse') {
      ...
      if (chunk.nonEmptyCellCount / CELLS_PER_CHUNK >= PROMOTE_TO_DENSE_FILL_RATIO) {
        ...
        for (const [i, cellValue] of chunk.localIndexToValue) {
          if (typeof cellValue === 'number') { dense.tagByLocalIndex[i] = 1; dense.numberByLocalIndex[i] = cellValue; }
          else if (typeof cellValue === 'boolean') { dense.tagByLocalIndex[i] = 3; dense.numberByLocalIndex[i] = cellValue ? 1 : 0; }
          else { dense.tagByLocalIndex[i] = 2; dense.stringIdByLocalIndex[i] = this.globalStringTable.getIdForString(String(cellValue)); }
        }
        dense.isDirty = true;
      ...
      if (typeof value === 'number') {
        ...
        chunk.stringIdByLocalIndex[localIndex] = this.globalStringTable.getIdForString(String(value));
      }
    }
  }
  ...
  deleteValue(globalRowIndex, globalColIndex) {
    ...
    if (chunk.kind === 'sparse') {
      if (chunk.localIndexToValue.delete(localIndex)) {
        chunk.nonEmptyCellCount--; chunk.isDirty = true;
      ...
      if (chunk.tagByLocalIndex[localIndex] !== 0) {
        ...
        if (chunk.nonEmptyCellCount > 0 &&
          ...
          for (let i = 0; i < CELLS_PER_CHUNK; i++) {
            ...
            else sparse.localIndexToValue.set(i, this.globalStringTable.getStringById(chunk.stringIdByLocalIndex[i]));
          }
          sparse.isDirty = true;
  ...
  setBlock(topRow, leftCol, values2D) {
    ...
    for (let r = 0; r < values2D.length; r++) {
      ...
      for (let c = 0; c < rowValues.length; c++) {
        ...
        if (cellValue === '' || cellValue == null) { this.deleteValue(globalRowIndex, globalColIndex); continue; }
        ...
        if (chunk.kind === 'sparse') {
          ...
          if (chunk.nonEmptyCellCount / CELLS_PER_CHUNK >= PROMOTE_TO_DENSE_FILL_RATIO) {
            const dense = createDenseChunk(); dense.nonEmptyCellCount = chunk.nonEmptyCellCount;
            for (const [i, v] of chunk.localIndexToValue) {
              if (typeof v === 'number') { dense.tagByLocalIndex[i] = 1; dense.numberByLocalIndex[i] = v; }
              else if (typeof v === 'boolean') { dense.tagByLocalIndex[i] = 3; dense.numberByLocalIndex[i] = v ? 1 : 0; }
              else { dense.tagByLocalIndex[i] = 2; dense.stringIdByLocalIndex[i] = this.globalStringTable.getIdForString(String(v)); }
            }
            dense.isDirty = true;
            this._hotChunks.set(this._lastAccessedChunkKey, dense);
        ...
        if (chunk.kind === 'dense') {
          if (typeof cellValue === 'number') {
            if (chunk.tagByLocalIndex[localIndex] === 0) chunk.nonEmptyCellCount++;
            chunk.tagByLocalIndex[localIndex] = 1; chunk.numberByLocalIndex[localIndex] = cellValue;
          } else if (typeof cellValue === 'boolean') {
            if (chunk.tagByLocalIndex[localIndex] === 0) chunk.nonEmptyCellCount++;
            chunk.tagByLocalIndex[localIndex] = 3; chunk.numberByLocalIndex[localIndex] = cellValue ? 1 : 0;
          } else {
            if (chunk.tagByLocalIndex[localIndex] === 0) chunk.nonEmptyCellCount++;
            chunk.tagByLocalIndex[localIndex] = 2; chunk.stringIdByLocalIndex[localIndex] = this.globalStringTable.getIdForString(String(cellValue));
          }
        }
        writeCount++;
      }
    ...
    for (const [chunkKey, chunk] of this._hotChunks.entries()) {
      ...
      if (chunk.kind === 'sparse') {
        ...
        for (let localIndex = 0; localIndex < CELLS_PER_CHUNK; localIndex++) {
          ...
          else jsValue = this.globalStringTable.getStringById(chunk.stringIdByLocalIndex[localIndex]);
          yield { row: baseGlobalRow + localRow, col: baseGlobalCol + localCol, value: jsValue };
        }
      }
    }
  ...
  *entriesInRect(topRow, leftCol, bottomRow, rightCol) {
  ...
  async flush() {
    ...
    if (this.globalStringTable.hasUnpersistedChanges) {
      tasks.push(this.chunkStore.putStringTableList(this.globalStringTable.stringById)
        .then(() => { this.globalStringTable.hasUnpersistedChanges = false; }));
    }
  ...
  estimatedBytesInHotCache() {
    ...
    for (const chunk of this._hotChunks.values()) {
      if (chunk.kind === 'sparse') {
        ...
        total += chunk.tagByLocalIndex.byteLength
